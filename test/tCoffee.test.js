...
//T-Coffee can be downloaded by running tCoffee_Wrapper/util/downloader.js
...
describe('#Align an unaligned sequence file', function () {
    it('should execute t_coffee command (sequence type = Protein)', function (done) {
        ...
        tcoffee.alignProtein('test/samples/sample_seq1.fasta',function (err) {
            if (err) {
                console.log(err);
            }
            else {
    ...
    it('should execute t_coffee command (sequence type = DNA)', function (done) {
        var inspect = stdout.inspect();
        tcoffee.alignDNA('test/samples/sample_dnaseq1.fasta',function (err) {
            if (err) {
                console.log(err);
            }
            else {
    ...
    it('should execute t_coffee command (sequence type = RNA)', function (done) {
        var inspect = stdout.inspect();
        tcoffee.alignProtein('test/samples/sample_rnaseq1.fasta',function (err) {
            if (err) {
                console.log(err);
            }
            else {
...
describe('#Align protein sequences using mcoffee mode', function () {
    it('should execute t_coffee command with no error', function (done) {
        var inspect = stdout.inspect();
        var inputFile =
        tcoffee.alignMcoffee('test/samples/sample_seq1.fasta',function (err) {
            if (err) {
                console.log(err);
            }
            else {
...
describe('#Align protein sequences using quickaln mode', function () {
    it('should execute t_coffee command with no error', function (done) {
        var inspect = stdout.inspect();
        var inputFile =
            tcoffee.alignProteinFast('test/samples/sample_seq1.fasta',function (err) {
                if (err) {
                    console.log(err);
                }
                else {
...
describe('#Align DNA sequences using procoffee mode', function () {
    it('should execute t_coffee command with no error', function (done) {
        var inspect = stdout.inspect();
        var inputFile =
            tcoffee.alignProCoffee('test/samples/sample_dnaseq1.fasta',function (err) {
                if (err) {
                    console.log(err);
                }
                else {
...
describe('#Align RNA sequences using rcoffee mode', function () {
    it('should execute t_coffee command with no error', function (done) {
        var inspect = stdout.inspect();
        var inputFile =
            tcoffee.alignRcoffee('test/samples/sample_rnaseq1.fasta',function (err) {
                if (err) {
                    console.log(err);
                }
                else {
